from .extract_question_text import app as question_extraction
...
async def extract_question(state: InputState) -> IntermediateState:
    result = await question_extraction.ainvoke({"text": state.text})  # type: ignore
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        results = await app.ainvoke(
            InputState(text=multi_question_text),
        )  # type: ignore
