...
async def test_create_question(
    question_manager_cloud, db_session, qpayload_min, tmp_path
):
    qcreated = await question_manager_cloud.create_question(qpayload_min, db_session)

    assert qcreated
    assert qcreated.title == qpayload_min["title"]

    assert Path(qcreated.blob_name) == Path("integration_test") / qpayload_min["title"]
...
async def test_create_question_duplicate(
    ...
    q1 = await question_manager_cloud.create_question(qpayload_min, db_session)
    logger.debug(f"This is q1{q1}")
    q2 = await question_manager_cloud.create_question(qpayload_min, db_session)
    
    logger.debug(f"This is q2{q2}")

    expected_path =  Path("integration_test")/ f"{qpayload_min['title']}_{q2.id}"
    assert Path(q2.blob_name) == expected_path
    
@pytest.mark.asyncio
async def test_get_question(question_manager_cloud, db_session, qpayload_min):
    q1 = await question_manager_cloud.create_question(qpayload_min, db_session)
...
@pytest.mark.asyncio
async def test_get_question_identifier(question_manager_cloud, db_session, qpayload_min):
    q1 = await question_manager_cloud.create_question(qpayload_min, db_session)
...
async def test_get_question_identifier_duplicate(
    question_manager_cloud, db_session, qpayload_min
):
    await question_manager_cloud.create_question(qpayload_min, db_session)
    q2 = await question_manager_cloud.create_question(qpayload_min, db_session)
...
async def test_save_file_to_question(
    question_manager_cloud, db_session, qpayload_min, file_data_payload
):
    q1 = await question_manager_cloud.create_question(qpayload_min, db_session)
...
@pytest.mark.asyncio
async def test_get_file(question_manager_cloud, db_session, qpayload_min, file_data_payload):
    q1 = await question_manager_cloud.create_question(qpayload_min, db_session)
...
@pytest.mark.asyncio
async def test_update_question(question_manager_cloud, db_session, qpayload_min):
    q1 = await question_manager_cloud.create_question(qpayload_min, db_session)
