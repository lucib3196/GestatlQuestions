...
const replace_params = (x, data) => {
  let st;
  while ((st = x.match("{{"))) {
    const stind = st.index;
    const enind = x.match("}}").index;
    const fullString = x.slice(stind, enind + 2);
    // console.log(`FullString is ${fullString}`);

    if (fullString.includes("params")) {
      const y = x.slice(stind + 9, enind);
      const d = _.get(data.params, y); 
      x = x.replace(fullString, d);
    } else if (fullString.includes("param_labels")) {
      const y = x.slice(stind + 15, enind);
      const d = data["param_labels"][y];
      x = x.replace(fullString, d);
    } else if (fullString.includes("correct_answers_labels")) {
      const y = x.slice(stind + 25, enind);
      const d = data["correct_answers_labels"][y];
      x = x.replace(fullString, d);
    } else if (fullString.includes("correct_answers")) {
      const y = x.slice(stind + 18, enind);
      const d = data["correct_answers"][y];
      x = x.replace(fullString, d);
    }
...
const pl_checkbox = ($, el) => {
  ...
  for (let j = 0; j < B.length; j++) {
    ...
    itemOrder.push(id);
    let s = `<input class ="response" type="checkbox" name="${nm}" id="${id}" value="${
      j + 1
    }">  <label for="${id}"> ${j + 1}. ${tx} </label> <br\> \n`;
...
const pl_multiple_choice = ($, el) => {
  ...
  for (let j = 0; j < B.length; j++) {
    ...
    itemOrder.push(id);
    let s = `<input type="radio" name="${nm}" class="response" id="${id}" value="${
      j + 1
    }">  ${j + 1}. ${tx} </br> \n`;
...
const pl_number_input = ($, qdata, el) => {
  ...
  let formEnd = "</fieldset> <div class='feedback'> </div> </form>";
  let htmlString = formOpen.concat(s);
  htmlString = htmlString.concat(formEnd);
  return htmlString;
};
...
const pl_hint = ($, qdata, el) => {
  ...
  return { htmlString: htmlString, level: level };
};
